...
// ── Theme ────────────────────────────────────────────────────────────
const T = (dark) => dark ? {
  bg: "#1c1c1e", desk: "#0d0d0f",
  card: "#26262a", cardSoft: "#1f1f22",
  line: "rgba(255,255,255,0.07)", lineSoft: "rgba(255,255,255,0.04)",
  text: "rgba(255,255,255,0.92)", dim: "rgba(255,255,255,0.55)", faint: "rgba(255,255,255,0.32)",
  side: "#222226", sideHover: "rgba(255,255,255,0.05)", sideActive: "rgba(255,255,255,0.10)",
  fill: "rgba(255,255,255,0.06)", fillStrong: "rgba(255,255,255,0.10)",
  pillBg: "rgba(255,255,255,0.10)",
} : {
  bg: "#fcfaf6", desk: "#d8d4cc",
  card: "#ffffff", cardSoft: "#f6f3ee",
  line: "rgba(60,50,30,0.10)", lineSoft: "rgba(60,50,30,0.05)",
  text: "#2c2924", dim: "#7a7368", faint: "#a8a196",
  side: "rgba(248,245,240,0.85)", sideHover: "rgba(60,50,30,0.04)", sideActive: "rgba(60,50,30,0.07)",
  fill: "rgba(60,50,30,0.04)", fillStrong: "rgba(60,50,30,0.07)",
  pillBg: "#ffffff",
};

// Soft accent palette — pastel-leaning, inspired by reference
const C = {
  green: "#34c466", amber: "#f5a524", red: "#e85a4f", blue: "#5b8def",
  purple: "#a98cf0", pink: "#e88ec1", cyan: "#5ec6d4", lilac: "#c4a8e8",
  warm: "#e89968", olive: "#9aa861",
};
...
const Stat = ({ t, label, value, icon: Ico, color = C.warm }) => (
  <div>
    ...
    <div style={{ fontSize: 22, fontWeight: 700, color, fontVariantNumeric: "tabular-nums", letterSpacing: -0.3 }}>{value}</div>
  </div>
);
  ...
  <div style={{
    ...
    <div style={{ display: "inline-flex", gap: 6, marginLeft: 2 }}>
      <ToggleChip t={t} on={sysProxy} onClick={() => setSysProxy(!sysProxy)}
        icon={Ic.web} label={`System Proxy · ${sysProxy ? "ON" : "OFF"}`} color={C.blue}/>
      <ToggleChip t={t} on={tunMode} onClick={() => setTunMode(!tunMode)}
        icon={Ic.shield} label={`TUN Mode · ${tunMode ? "ON" : "OFF"}`} color={C.warm}/>
      <ToggleChip t={t} on={anon} onClick={() => setAnon(!anon)}
        icon={anon ? Ic.eyeOff : Ic.eye}
        label={`Anonymous Mode · ${anon ? "ON (info masked)" : "OFF"}`} color={C.purple}/>
    </div>
  </div>
);
    ...
    <div style={{ padding: "16px 18px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      ...
      <Card t={t} title="Network Status" icon={Ic.web} iconColor={C.purple}
        ...
        <div style={{ borderTop: `0.5px solid ${t.lineSoft}`, paddingTop: 12,
          ...
          <SubStat t={t} icon={Ic.cube}   label="Local IP" value="CN 海西…168.157.83"/>
          <SubStat t={t} icon={Ic.globe}  label="Proxy IP" value="HK Hong…249.100.80"/>
        </div>
      </Card>
      ...
      <Card t={t} padding={0}>
        ...
        {RULES.map((r, i) => (
          <div key={i} style={{
            ...
            <span style={{ color: t.text, fontFamily: "ui-monospace, SF Mono, Menlo", fontSize: 11.5 }}>{r.match}</span>
        ...
        <div className={"sc" + (dark ? " dk" : "")} style={{ flex: 1, overflowY: "auto" }}>
          {conns.map(c => (
            <div key={c.id} style={{
              ...
              <span style={{ color: t.text, fontFamily: "ui-monospace, SF Mono, Menlo", fontSize: 11, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.host}</span>
...
function App() {
  ...
  const screenEl = (() => {
    switch (screen) {
      ...
      case "advanced":    return <StubScreen t={t} title="Advanced" sub="DNS, ports, kernel options."/>;
    }
  })();
    ...
    <div style={{ position: "fixed", inset: 0, background: t.desk, padding: 28, boxSizing: "border-box" }}>
      <div style={{
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div className={"sc" + (dark ? " dk" : "")} style={{ flex: 1, overflowY: "auto", color: t.text }}>
            {screenEl}
          </div>
        </div>
      </div>
